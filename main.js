...
async function transcribeWithAssemblyAI(apiKey, blob, selectedLang) {
  try {
    ...
    let uploadRes;
    try {
      uploadRes = await (0, import_obsidian2.requestUrl)({
    ...
    let jobRes;
    try {
      jobRes = await (0, import_obsidian2.requestUrl)({
    ...
    for (let i = 0; i < 30; i++) {
      ...
      let pollRes;
      try {
        pollRes = await (0, import_obsidian2.requestUrl)({
...
var SpeakNotePlugin = class extends import_obsidian3.Plugin {
  constructor() {
    super(...arguments);
    ...
    this.addCommand({
      ...
      callback: async () => {
        if (this.lastSavedFile) {
          ...
          new import_obsidian3.Notice("No recent recording found");
        }
      }
    });
  }
  ...
  async toggleRecording() {
    if (this.isBusy) {
      new import_obsidian3.Notice("Please wait");
      return;
    }
    this.isBusy = true;
    ...
    try {
      ...
      new import_obsidian3.Notice("Failed to toggle recording");
    } finally {
    ...
    if (this.mediaRecorder && this.isRecording) {
      try {
        ...
        if (this.isAutoStopped) {
          ...
          new import_obsidian3.Notice("Recording stopped, saving file");
        }
      } catch (err) {
        console.error("Error while stopping recording:", err);
        new import_obsidian3.Notice("Could not stop recording cleanly");
      }
    }
  }
  async saveRecording(blob) {
    ...
    try {
      ...
      new import_obsidian3.Notice(`Saved: ${newFile.path}`);
      if (this.settings.autoTranscribe) {
        ...
        if (text) {
          ...
          new import_obsidian3.Notice(`Transcript saved: ${transcriptFile.path}`);
        } else {
          new import_obsidian3.Notice("Empty transcription result");
        }
      }
    } catch (apiError) {
      this.hideOverlay();
  ...
  async playRecording(file) {
    try {
      if (!this.app.vault.getAbstractFileByPath(file.path)) {
        new import_obsidian3.Notice("Recording no longer exists");
        return;
      }
      ...
      new import_obsidian3.Notice(`Playing ${file.name}`);
    } catch (err) {
  ...
  handleError(source, err) {
    ...
    if (msg.includes("language") || msg.includes("unsupported")) {
      new import_obsidian3.Notice("Language not supported by this provider");
      return;
    }
